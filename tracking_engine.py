...
from gesture_state import HeldPose, PinchGesture, PointerFilter, PointerStabilizer, ScrollGesture
from hand_tracker import HandTracker, draw_landmarks
from pointer_output import PointerOutput

log = logging.getLogger(__name__)

INFERENCE_WIDTH = 640          # frames are shrunk to this before hand tracking
...
class TrackingEngine(threading.Thread):
    ...
    def _make_tracker(self) -> bool:
        try:
            ...
            log.exception("Hand tracker failed to start")
    ...
    def _on_camera_status(self, state: str, detail: Any) -> None:
        ...
        if state == "connected" and isinstance(detail, CameraCapabilities):
            self._label = "Show one hand to the camera"
        elif state == "error":
    ...
    def _process(self, image: np.ndarray, captured_at: float) -> None:
        ...
        landmarks = None
        try:
            landmarks = self.tracker.process(rgb)
            self.inference_errors = 0
        except Exception:
            self.inference_errors += 1
            log.exception("Hand inference failed (%d in a row)", self.inference_errors)
    ...
    def _rebuild_tracker(self) -> None:
        log.warning("Rebuilding the hand tracker after repeated errors")
        try:
            if self.tracker is not None:
                self.tracker.close()
        except Exception:
        ...
        try:
            ...
            log.exception("Hand tracker rebuild failed")
